...
def rebuild(srcdir,out,expected,expected_parts):
    parts=sorted(srcdir.glob('part-*.txt'))
    if len(parts)!=expected_parts: raise SystemExit(f'{srcdir}: expected {expected_parts} parts, got {len(parts)}')
...
rebuild(WF_DIR,WF_OUT,EXPECTED_WF,17)
rebuild(PL_DIR,PL_OUT,EXPECTED_PL,2)
